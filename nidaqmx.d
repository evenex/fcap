...
struct DAQmx
	{/*...}*/
		__gshared static nothrow:

		template opDispatch (string op, uint line = __LINE__)
			{/*...}*/
				version (NO_DAQ)
					void opDispatch (Args...)(Args args)
						{/*...}*/
							try {/*...}*/
								stderr.write (`called ` ~op~ ` ` ~Args.stringof~ ` with `);

								foreach (arg; args)
									stderr.write (arg.text~ ` `);

								stderr.writeln (`on line ` ~line.text);
							}
							catch (Exception) assert (0);
						}
				else void opDispatch (Args...)(Args args)
					{/*...}*/
						static assert (__traits(compiles, mixin(q{DAQmxBase} ~op~ q{ (args)})),
							`call to ` ~op~ ` (` ~args.text~ `) failed to compile`
						);

						mixin(q{
							uint status = DAQmxBase} ~op~ q{ (args);
						});

						if (status != DAQmx_Success)
							{/*...}*/
								char[512] error_string;

								DAQmxBaseGetExtendedErrorInfo (error_string.ptr, error_string.sizeof);
								stderr.writeln (`call to ` ~op~ ` (` ~args.text~ `) failed: `, error_string);
